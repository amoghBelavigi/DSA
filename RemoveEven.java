...
//keep only odd integers in an array
...
public class Main
{
    ...
    public static int[] removeEven(int[] arr){  //u have to return the array so "int[]" is used
        int oddCount=0;  //to count the number of odd numbers present in the array
        for(int i=0;i<arr.length;i++){
            if(arr[i]%2!=0){ //to check if the integer is odd or no
                oddCount++;  //if odd then increment the oddCount
            }
        }
        int[] result=new int[oddCount];  //create a new result array to store only the odd elements of the array
